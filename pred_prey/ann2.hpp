...
namespace ann {
  ...
  struct Neuron
  {
    ...
    static constexpr size_t state_size = bias_state_size + weights_state_size + activation_state_size + feedback_state_size;

    // state layout: { [bias weight], #input weights, [activation state], [feedback state] }
    static constexpr size_t bias_begin = 0;
    static constexpr size_t weights_begin = bias_state_size;
  ...
  public:
    ...
    bias_view_t bias() noexcept { return bias_view_t(state_); }
